...
maxVal=r_max_l*c_max_l;
cr1temp=zeros(3,maxVal);
k=1;
while k<maxVal+1
    for i=1:r_max_l
        for j = 1:c_max_l
            cr1temp(:,k) = Xl_inv*[j i 1]';
            cr1temp(:,k) = round(cr1temp(:,k)/cr1temp(3,k));
...
cr1temp(1,:) = cr1temp(1,:) - 300;
k=1;
while k<maxVal+1
    for i=1:r_max_l
        for j = 1:c_max_l
            if ((cr1temp(2,k)>0) && (cr1temp(1,k)>0) && (cr1temp(2,k)<r_max_l) && (cr1temp(1,k)<c_max_l))
                newL(i,j) = left_pattern(cr1temp(2,k),cr1temp(1,k));
            end
            k = k+1;
        end
    end
